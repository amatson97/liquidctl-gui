...
## Requirements
- Python 3
- Python development headers for your distro's Python package when `liquidctl` needs to build native dependencies (for example `python3-dev` or `python3.14-dev` on Debian/Ubuntu-like systems)
...
- If your distro's Python headers are missing, the launcher will try to install the matching `python*-dev` package before building `liquidctl`.
